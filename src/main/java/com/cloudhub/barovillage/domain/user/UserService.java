...
public class UserService {
    ...
    @Transactional
    public Boolean checkUserLocationStatus(Long loginUserId) {
        User user = userRepository.findById(loginUserId)
                .orElseThrow(() -> new IllegalArgumentException("존재하지 않는 유저입니다."));

        // 유저 위치인증 상태 업데이트 및 확인
        List<UserLocation> activeLocation = userLocationRepository.findAllByUserAndStatus(user, ACTIVE).stream().map(ul -> {
                    return updateUserLocationStatus(ul); // 업데이트
                }).filter(updatedUserLocation -> updatedUserLocation.getStatus() == ACTIVE)
                .toList();

        if (activeLocation.isEmpty() || activeLocation.size() <= 0) // ACTIVE인 위치 인증 기록이 없다면
            return false;
        else // 있다면
            return true;
...
//
//        UserLocation userLocation = userLocationRepository.save(UserLocation.builder()
//                .user(user)
//                .latitude(latitude)
//                .longitude(longitude)
...
//
//        for(int i = addressList.size()-1 ; i < -1 ; i--) {
